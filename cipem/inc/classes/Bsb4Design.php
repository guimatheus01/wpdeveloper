    ...
    class Bsb4Design
    {
        ...
        public function pagination($pagination_align_class = 'justify-content-center', $return = false)
        {
            ...
            if (is_array($pagination_array) && !empty($pagination_array)) {
                $output .= '<nav class="environment-pagination">';
                $output .= '<ul class="page-numbers ' . $pagination_align_class . '">';
                foreach ($pagination_array as $page) {
                    $output .= '<li';
                    if (strpos($page, '<a') === false && strpos($page, '&hellip;') === false) {
                        $output .= ' class="page-numbers current"';
                    } else {
                        $output .= ' class="page-numbers"';
                    }
                    $output .= '>';
                    if (strpos($page, '<a') === false && strpos($page, '&hellip;') === false) {
                        $output .= '<a class="page-numbers">' . $page . '</a>';
                    } else {
                        if (strpos($page, 'class=') === false) {
                            $page = str_ireplace('<a', '<a class="page-numbers"', $page);
                        } else {
                            $page = str_ireplace('class="', 'class="page-numbers ', $page);
                            $page = str_ireplace('class=\'', 'class=\'page-numbers ', $page);
                        }
                        $output .= $page;
                    }
